// Sample type --> Package, Referral type --> Doctor, partial bill payment in registration, Full Bill Paid using Due clearance
// Check Email Check box field, No discounts, Partial approval then full approval
...
public class S102
{
	...
	@Test(priority = 1)
	public void s002() throws Exception
	{
		...
		try 
		{
			...
			Assert.assertEquals(MasterControlObject.getAlertMsg(), "Service Not Completed","***Alert msg not displayed as expected after click email***");
			MasterControlObject.acceptAlert();
			
			//Approve unapproved results in Manage Approval
			MasterControlObject.changeRole("PATHOLOGIST");
			ManageApprovalObj.searchRecord(sFirstName);
			ManageApprovalObj.clickSave();
			Assert.assertEquals(MasterControlObject.getAlertMsg(),"Results Approved Successfully","***Alert not displayed as expected after approving results***");
			...
			manageDispatchobj.verifyModeOfDispatch("EMAIL");
		}
