...
public class S104_CheckDoctorPayOffWhenDiscountTypeDoctorApprovedByDoctorAndItemLevelDiscountInAmount
{
	...
	@Test(priority = 1)
	public void s104_CheckDoctorPayOffWhenDiscountTypeDoctorApprovedByDoctorAndItemLevelDiscountInAmount() throws Exception
	{
		try 
		{
			...
			String sItemDiscountType = ExcelUtils.getCellData(iTestCaseRow, Constant.col_ItemDiscountType);
			String sItemDiscountValue = ExcelUtils.getCellData(iTestCaseRow, Constant.col_ItemDiscountValue);
...
//			generateBillPageObject.selectReferralName(sReferralName);
			...
			float grossamount = generateBillPageObject.getGrossAmount();
			float DiscountValue = Float.parseFloat(sItemDiscountValue);
			float discountamount = grossamount*DiscountValue/100;
			generateBillPageObject.EnterItemDiscountValue(1, Float.toString(discountamount));
			...
			float paypercent = Float.parseFloat(sValue) - Float.parseFloat(sItemDiscountValue);
			doctorPayoffReport.verifyPayOffAmount(Float.toString(paypercent));
