...
public class S102_CheckDoctorPayOffWhenDiscountTypeDoctorApprovedByDoctorAndDiscountInAmount
{
	...
	@Test(priority = 1)
	public void s102_CheckDoctorPayOffWhenDiscountTypeDoctorApprovedByDoctorAndDiscountInAmount() throws Exception
	{
		try 
		{
...
//			generateBillPageObject.selectReferralName(sReferralName);
			...
			float DiscountValue = Float.parseFloat(sBillDiscountValue);
			float discountamount = grossamount*DiscountValue/100;
			generateBillPageObject.enterBillDiscount(Float.toString(discountamount));
			...
			float paypercent = Float.parseFloat(sValue) - Float.parseFloat(sBillDiscountValue);
