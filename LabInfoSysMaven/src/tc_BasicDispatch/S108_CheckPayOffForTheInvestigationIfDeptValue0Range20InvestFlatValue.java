...
public class S108_CheckPayOffForTheInvestigationIfDeptValue0Range20InvestFlatValue
{
	...
	@Test(priority = 1)
	public void s108_CheckPayOffForTheInvestigationIfDeptValue0Range20InvestFlatValue() throws Exception
	{
		try 
		{
			...
			//Admin Login
			loginPageObject.login(Constant.GPLAdmin, "Admin@123");
			Assert.assertEquals("Analyte Master", driver.getTitle(),"Login failed");
			MasterControlObject.gotoPage("Doctor PayOff");
			...
			doctorPayoff.enterValueForInvestigationForFlat(sInvestigationName, sPayoutValue);
			Assert.assertEquals(MasterControlObject.getAlertMsg(), "Updated Successfully", "*** Doctor payoff --> Alert not shown as expected ***");
			MasterControlObject.acceptAlert();
			...
			doctorPayoff.verifyPayoutTypeForInvestigation(sInvestigationName, sPayoutType);
			doctorPayoff.verifyValueForInvestigation(sInvestigationName, sPayoutValue);
...
//			generateBillPageObject.selectReferralName(sReferralName);
			...
			generateBillPageObject.EnterItemDiscount(1,sItemDiscountType1,sItemDiscountValue1);
			...
			float totaldiscountvalue = Float.parseFloat(sItemDiscountValue1)+Float.parseFloat(sItemDiscountValue2);
			doctorPayoffReport.verifyPayOffAmount(Float.toString(totaldiscountvalue));
