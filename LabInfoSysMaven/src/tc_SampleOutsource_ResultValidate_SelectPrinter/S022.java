...
public class S022 
{
	...
	@Test(priority = 1)
	public void s022() throws Exception
	{
		try 
		{
...
//			Assert.assertEquals(Constant.WHITE, manageSamplePageObjects.getRowColor(sServiceName),"*** Color not showing as expected ***");
			...
			Assert.assertEquals(MasterControlObject.getAlertMsg(), "Results Saved Successfully","*** Alert not shown as expected after RESULT VALIDATION ***");
			MasterControlObject.acceptAlert();
			MasterControlObject.gotoPage("Patient List");
			patientListPageObjects.searchRecord(sFirstName);
			...
			manageSamplePageObjects.searchRecord(sFirstName);
			manageSamplePageObjects.selectPatient();
//			Assert.assertEquals(manageSamplePageObjects.getRowColor(sServiceName), Constant.GREEN,"*** Color not showing as expected ***");
			...
			//Verify record in Sample Outsource
			MasterControlObject.gotoPage("Sample Outsource");
			Assert.assertEquals(sampleOutsource.isPatientDisplayed(sFirstName),"TRUE","*** Patient Name not showing in Outsourced test list ***");
			Assert.assertEquals(sampleOutsource.isInvestigationDisplayed(sFirstName, sServiceName),"TRUE","*** Sample not showing correctly in Outsourced test list ***");
			sampleOutsource.viewNonOutsourcedTest();
			Assert.assertEquals(sampleOutsource.isPatientDisplayed(sFirstName),"FALSE","*** Patient Name showing in Non Outsourced test list ***");
			sampleOutsource.viewOutsourcedTest();
			sampleOutsource.selectPatient(sFirstName);
			sampleOutsource.selectOutsourcingTo(sOutsourcingTo);
			sampleOutsource.clickSampleOutsource();
			Assert.assertEquals( MasterControlObject.getAlertMsg(),"Sample Outsourced Successfully","***Sample Outsource --> Alert msg not displayed as expected.***");
