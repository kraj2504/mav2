...
public class LogoutAsReceptionist
{
	...
	@Test(priority = 1)
	public void logoutAsReceptionist() throws Exception
	{
		...
		PO_MasterControl masterControlObjects = PageFactory.initElements(driver,PO_MasterControl.class);
		...
		try 
		{
			...
			masterControlObjects.ClickSignOut();

			Assert.assertEquals(driver.getTitle(),"Login","***Logout failed***");
		}
	...
	public void updateResult(ITestResult result) throws Exception
	{
		if(result.getStatus() == ITestResult.SUCCESS)
		{
			...
			Log.info("Logout successfully ==> Test Passed");
		}
		else if(result.getStatus() == ITestResult.FAILURE)
		{
			ExcelUtils.setCellData("Fail", iTestCaseRow, Constant.col_Result);
			Log.info("Logout failed ==> Test Failed");
			Utils.takeScreenshot(driver, sTestCaseName);
		}
