...
public class BulkEntry_VerifyStatusAfterResultSubmission
{
	...
	@Test(priority = 1)
	public void verifyStatusAfterResultSubmission() throws Exception
	{
		...
		{		 
			...
			//Verify status in Patient List
			MasterControlObject.gotoPage("Patient List");
			patientListPageObject.searchRecord(sFirstName);
			
			Assert.assertEquals(patientListPageObject.getStatus(),"Result Entered","***Status not updated as expected***");
		}
	...
	public void updateResult(ITestResult result) throws Exception
	{
		if(result.getStatus() == ITestResult.SUCCESS)
		{
			...
			Log.info("Status showing as 'Result Entered' ==> Test Passed");
		}
		else if(result.getStatus() == ITestResult.FAILURE)
		{
			ExcelUtils.setCellData("Fail", iTestCaseRow, Constant.col_Result);
			Log.info("Status not showing as 'Result Entered' ==> Test Failed");
			Utils.takeScreenshot(driver, sTestCaseName);
		}
