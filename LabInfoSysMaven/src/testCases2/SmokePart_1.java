...
public class SmokePart_1
{
	...
	//Verifying Upload TRF in Registration - Not possible to automate
	/*@Test(priority = 6) 
	public void uploadTRFinGenerateBill() throws Exception
	{	
		try
		{
			Log.info("*** Verifying Upload TRF in Registration ***");
			 PO_Registration generateBillObj = PageFactory.initElements(driver,PO_Registration.class);
			 PO_MasterControl MasterControlObj = PageFactory.initElements(driver,PO_MasterControl.class);
			 		 
			 generateBillObj.uploadTRF("Upload TRF");
			 Assert.assertEquals(MasterControlObj.getAlertMsg(),"TRF Upload Successfully","***TRF Upload failed***");
			 MasterControlObj.acceptAlert();
			 generateBillObj.clickFileUploadtool();
		}
